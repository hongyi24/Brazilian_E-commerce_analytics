...
from Cross_regional_sales where seller_state is not null;

--跨州订单的物流成本是否显著更高？
SELECT
    ROUND(AVG(IF(customer_state != seller_state, 1, 0) * freight_cost), 2) AS Cross_freight_cost,
    ROUND(AVG(IF(customer_state = seller_state, 1, 0) * freight_cost), 2) AS Same_freight_cost
FROM
    Cross_regional_sales
WHERE
    seller_state IS NOT NULL;-- 17.742218280870947， 5.825033474319637