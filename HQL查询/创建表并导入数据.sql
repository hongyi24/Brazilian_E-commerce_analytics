create table customers(
    customer_id string comment '每个订单都有唯一的customer_id',
    customer_unique_id string comment '客户的唯一标识符',
    customer_zip_code_prefix string comment '客户邮政编码的前 5 位数字',
...
create table geolocation (
    geolocation_zip_code_prefix string comment '客户邮政编码的前 5 位数字',
...
create table Order_Items(
    order_id string comment '订单唯一标识符',
    order_item_id int comment 'Sequential Number （序列号） 标识同一订单中包含的项目数',
    product_id string comment '商品唯一标识符',
    seller_id string comment '卖家唯一标识符',
...
create table Order_Payments(
    order_id string comment '订单的唯一标识符',
...
create table Order_Reviews(
    review_id string comment '唯一评价标识符',
    order_id string comment '唯一订单标识符',
    review_score int comment '备注，范围从 1 到 5 不等，由客户在满意度调查中提供',
    review_comment_title string comment '客户留下的评论的评论标题（葡萄牙语）',
    review_comment_message string comment '客户留下的评论中的葡萄牙语评论消息',
...
create table Order_Dataset(
    order_id string comment '订单的唯一标识符',
    customer_id string comment 'key 添加到 Customer 数据集中。每个订单都有唯一的customer_id',
    order_status string comment '对订单状态（已送达、已发货等）的引用',
...
create table Products (
    product_id string comment '唯一商品标识码',
    product_category_name string comment '产品的根类别（葡萄牙语）',
...
create table Sellers (
    seller_id string comment '卖家唯一标识符',
    seller_zip_code_prefix string comment '卖家邮政编码的前 5 位数字',
    seller_city string comment '卖家城市名称',
    seller_state string comment '卖家状态'
)
...
create table Category_Name_Translation(
    ...
    product_category_name_english string comment '英文类别名称'
)
